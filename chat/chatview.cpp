	...
	, isAppended(false)
{
	...
	pScrollArea->setObjectName("chat_area");
	...
	pScrollArea->setWidget(w);    //应该时在QSCrollArea构造后执行 才对

	pScrollArea->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
	QScrollBar* pVScrollBar = pScrollArea->verticalScrollBar();
	...
	pScrollArea->setLayout(pHLayout_2);
...
{
	QVBoxLayout* vl = qobject_cast<QVBoxLayout*>(pScrollArea->widget()->layout());
...
{
	QVBoxLayout* layout = qobject_cast<QVBoxLayout*>(pScrollArea->widget()->layout());
...
bool ChatView::eventFilter(QObject* o, QEvent* e)
{
	...
	else */if (e->type() == QEvent::Enter && o == pScrollArea)
	{
		pScrollArea->verticalScrollBar()->setHidden(pScrollArea->verticalScrollBar()->maximum() == 0);
	}
	else if (e->type() == QEvent::Leave && o == pScrollArea)
	{
		pScrollArea->verticalScrollBar()->setHidden(true);
	}
